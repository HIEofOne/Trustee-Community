...
if [ -d "${HOME}/.nvm/.git" ]; then
  ...
  cp ./docker/traefik/routes.tmp ./docker/traefik/routes/routes.yml
  ...
  sed -i "s/example.com/$ROOT_DOMAIN/" ./docker/traefik/routes/routes.yml
  ...
  sed -i '/^NEXT_PUBLIC_COUCH_USERNAME=/s/=.*/='"admin"'/' ./docker/trusteecommunity/.env.local
  sed -i '/^NEXT_PUBLIC_COUCH_PASSWORD=/s/=.*/='"$COUCHDB_PASSWORD"'/' ./docker/trusteecommunity/.env.local
  ...
  sed -i '/^NEXT_PUBLIC_MAGIC_PUB_KEY=/s/=.*/='"$NEXT_PUBLIC_MAGIC_PUB_KEY"'/' ./docker/trusteecommunity/.env.local
  ...
  DIGITALOCEAN_PUB_KEY=$(cat /root/.ssh/id_ed25519.pub)
  DIGITALOCEAN_SSH_KEY_ID=`curl -X POST \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $DIGITALOCEAN_API_TOKEN" \
  -d '{"name":"Trustee Root Public Key","public_key":"$DIGITALOCEAN_PUB_KEY"}' \
